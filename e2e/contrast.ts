 ...
 *
 *  - text over a *gradient* — axe declines to compute a ratio and files the
 *    node under `incomplete`. `body` paints two `radial-gradient` washes over
 *    `--bg`, the budget meter fills with a `linear-gradient`, and the hero's
 *    "why it matters" aside is a `color-mix(in oklab, ...)` tint, so text on any
 *    of them composites onto something no flat colour describes.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. `.btn:disabled` renders its
 *    whole subtree at `opacity: .45`, and the plot's ghost curves at `.4`.
 *
 ...
 * the helper report a ratio nothing on screen has, and all three are live here:
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. `.table-scroll` is
 *    an `overflow-x: auto` scroller around the recovered-vs-true coefficient
 *    table, whose `--bg-inset` cells hold 20-digit field elements; at 380px most
 *    of its width is clipped away. Content scrolled past the client box is not
 *    dimmed or partly drawn — it is absent from the frame, and asking what
 *    colour it sits on has no answer. Its rect is still to the right of every
 ...
 *    viewport where the very same cell is visible and measured for real.
 *
 ...
 *    never laid down.
 *
 *  - SVG PAINTS IN DOCUMENT ORDER, SO SIBLINGS CAN BE THE BACKGROUND. Both of
 *    this lab's figures are inline SVG with real `<text>`: the cliff plot and
 *    the soft-vs-sharp comparison, whose "within budget" label is a `<text>`
 *    drawn after the `<rect class="cmp-safe">` band it sits inside. A label's
 *    backdrop there is a PRECEDING SIBLING shape, not an ancestor. An
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     * element and badly wrong here: `body` paints two low-alpha radial washes
     * that fade to `transparent` by ~42% — decorative glows anchored at the top
     * corners. Judging the closing sections, several page-heights below, by the
     * corner colour would invent backdrops that text never sits on. So each
     ...
     * unmeasurable and paints nothing — which is also why the `select` chevron,
     * a `url("data:image/svg+xml,...")` background layer, contributes nothing
     * here and is hand-measured instead.
     */
     ...
     *
     * A driven page here holds a coefficient table with two field elements per
     * row, a ledger of point chips, two SVG figures' worth of labels and six
     * panels of prose, and every one of them walks the same handful of ancestors
     * up to `.panel`, `main` and `body`. Without the caches the pass re-reads the
     * same computed styles and rects thousands of times. Nothing mutates the DOM
     ...
     * This is not hypothetical here: `.sr-only` in this lab is exactly
     * `clip: rect(0 0 0 0)`, and it wraps the coefficient table's `<caption>`
     * and the "match" column header. Without this guard the walk composites that
     * text against whatever surface it happens to sit on and INVENTS failures
     * for text nobody can see. Deliberately narrow — only a ZERO-area clip
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page's disclosure is the `.glossary`, whose open
      // state is a real state the gate scans explicitly by clicking its summary.
      ...
      // slides to `top: 0` only on focus. (`src/style.css` also carries a
      // `.skip-link { top: -100px }` rule for the same idiom, but nothing on the
      // page has that class — it is dead, and the shared header's link is what
      // actually satisfies 2.4.1.) Measuring the parked copy invents a failure
     ...
     * stricter than axe on decorative content. Honour the same boundary. It is
     * load-bearing here: every panel's `.panel-num` ("01".."06") is
     * `aria-hidden`, as is the theme button's 🌙 — and an emoji paints in its own
     * multicolour rather than in the element's `color`, so measuring one reports
     * a ratio describing nothing on screen.
     */
